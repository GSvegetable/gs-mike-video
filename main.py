...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # 处理用户点击分享链接进来的情况（比如 t.me/bot?start=变形金刚）
    args = context.args
    if args:
        video_id = args[0]
...
async def upload_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 检查有没有带上名字，比如 /upload 变形金刚
    if not context.args:
        await update.message.reply_text("❌ 格式错误。正确格式：`/upload 电影名字`", parse_mode='Markdown')
        return
